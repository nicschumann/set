...
<<<<<<< HEAD
	/**
	 * A geometry set can be instantiated over a specific dimension. Instantiating a geometry also
	 * determines equivalence classes over members of that geometry, and constituted an inductive
	 * definition of the geometry on a specific dimension. This is a way of introducing type level
	 * computation into the java hierarchy.
	 * 
	 * @param dimension
	 *            the dimension of the bases of this geometry; valid for dimension > 0;
	 */
	public VectorSpace(int dimension, Gensym g) {
	...
	public abstract class Geometry {
		/**
		 * This method returns a the name of this geometric object. Each object is required to have
		 * a unique name by the runtime system, so that the CSP can track it, and so that the
		 * language can manipulate it. If the user does not specify the name of the symbol, the
		 * runtime system generates one for it. This allows equality on geometries to be decided
		...
		/**
		 * this method returns the atomic names that are bound to values in this geometry. for
		 * example, a vector X in R3 might have values x1 x2 x3. for X, values() returns the map (
		 * x1 -> n1, x2 -> n2, x3 -> n3 ), where the n_i are the numerical values of the x_i.
		 * 
		...
		/**
		 * this method returns the set of components that this geometry has named. for example, a
		 * vector X in R3 might have the component set { x1, x2, x3 }.
		 * 
		...
		/**
		 * This method returns the dimension of the VectorSpace in which this geometry is defined.
		 * 
	...
	/**
	 * Basis of the geometric structure. This class represents a location in n-dimensional space
	 */
	public class Point extends Geometry {
		public Point(Symbol name, Map<Symbol, DoubleReference> components) throws GeometricFailure {
			if (components.size() != dimension)
				throw new GeometricFailure(components.size());
		...
		public Point(Symbol name, DoubleReference... components) throws GeometricFailure {
			if (components.length != dimension)
				throw new GeometricFailure(components.length);
		 ...
		 * 
		 * for an array of components in this vector, x1 ... xn, select a component with subscript i
		 * and return it.
		 * 
		 * @param i
		 *            the subscript of the component to select
		 * @return the double box represented by the corresponding X_i
		 */
		public DoubleReference getN_(int i) throws GeometricFailure {
			if (i <= 0 || i > dimension)
				throw new GeometricFailure(i);
		...
		/**
		 * given a geometry in this vector space, join attempts to map elements of this geometry
		 * onto the supplied one. Given two structurally identical geometries, this method returns a
		 * mapping from the symbols of the first into the symbols of the second, ie, an image of the
		 * first in the second. If the two structures are not similar, no such mapping exists, and
		 * the empty map is returned
		 * 
		 * @param x
		 *            a geometry in the vector space to attempt to join.
...
=======
    ...
    public class Point extends Geometry {
        ...
        private Symbol name;
        private Symbol[] names = new Symbol[ dimension ];
        private MDouble[] components = new MDouble[ dimension ]; // TODO ensure safety
